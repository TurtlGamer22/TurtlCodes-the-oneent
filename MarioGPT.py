...
# Adjust the `check_platform_collision` function accordingly
...
def restart_game():
    global platforms, score, player_pos, on_ground, enemies, projectiles, jumps_left
    player_pos = [50, SCREEN_HEIGHT - player_size - 100]
    platforms = [[0, SCREEN_HEIGHT - 100, SCREEN_WIDTH, 100]]
    enemies = []
    projectiles = []
...
def check_platform_collision():
    global player_pos, player_velocity, on_ground, jumps_left
    player_rect = pygame.Rect(player_pos[0], player_pos[1], player_size, player_size)
    for plat in platforms:
        plat_rect = pygame.Rect(plat[0], plat[1], plat[2], plat[3])
        if player_rect.colliderect(plat_rect) and player_velocity >= 0:
            ...
            player_velocity = 0
            on_ground = True
            jumps_left = 2  # Reset double jump on landing
            return
